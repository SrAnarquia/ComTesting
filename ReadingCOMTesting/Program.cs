...
using System.Data.SQLite; //THis is to manage the smaller db on next versions
using System.IO; 
...
namespace devicesCOM
{
    class ProgramPort
    {
        ...
        //Azure connection
        static string connectionString = "Server=tcp:servidordeployment.database.windows.net,1433;Initial Catalog=KIA_SURVEY;Persist Security Info=False;User ID=SysAdmin;Password=Kia4ppSurv3y!;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=3;";

        //Local connection
        //static string connectionString = "Server=DESKTOP-1FQINI4\\SQLEXPRESS;Database=Hardware;Trusted_Connection=True;Connect Timeout=3;";
        ...
        static async Task Main(string[] args)
        {
            ...
            try
            {
                ...
                {
                    string insertLogQuery = "INSERT INTO dataAcquisitionLogs (DATE_TIME_INIT,[Status]) OUTPUT INSERTED.ID VALUES (@startTime,@Status)";
            ...
            if (sP.IsOpen)
            {
                ...
                if (sqlServerAvailable)
                {
                    try
                    {
                        using (SqlConnection conn = new SqlConnection(connectionString))
                        {
                            //Status 0 equals not active, 1 active and null not finished
                            string updateQuery = "UPDATE dataAcquisitionLogs SET DATE_TIME_FINISHED = @endTime,[Status]=@Status WHERE ID = @logId";
